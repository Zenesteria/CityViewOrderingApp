...
const Product = ({soup}) => {
  ...
  const [currentDisplayBg, setCurrentDisplayBg] = useState(`url('${soup.imgs[0]}')`)
        ...
        <div className="flex w-full min-h-[80vh] justify-center h-fit ml-auto flex-wrap">
          <div className="flex flex-col h-[80vh] max-h-[1000px] min-w-[360px] p-4 flex-[0.7]">
            ...
            <div className="flex w-full h-[15%] items-center justify-center">
              <DisplayButton bg={`url('${soup.imgs[0]}')`} mouseOver={handleTempBg}/>
              <DisplayButton bg={`url('${soup.imgs[1]}')`} mouseOver={handleTempBg}/>
            </div>
          </div>
          <div className="flex flex-col p-4 h-full min-w-[300px] flex-[1]">
            <h1 className='text-[2rem] font-bold mb-2'>{soup.title}</h1>
            <p className='text-[1.2rem] font-semibold italic mb-2'>Category: {soup.category}</p>
            ...
            <div className="flex flex-col w-fit my-4 p-2">
                <p className='font-bold text-[1.5rem]'>{`₦${soup.prices[0]}.00`}</p>
