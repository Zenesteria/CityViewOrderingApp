  ...
  return (
    <div className="flex w-full h-fit flex-col">
        <div className="flex w-full h-[50vh] relative justify-center items-center p-5">
            ...
            <h1 className='text-[4rem] text-white font-bold'>City View Cocktails</h1>
        </div>
        <div className="flex w-full flex-wrap p-5 items-center">
            {drinksdata.map((drink) => {
                return(
                    <ProductDisplay key={drink._id} data={drink} path={'/product/cocktails/'}/>
                )
            })}
        </div>
    </div>
...
export const getServerSideProps = async() => {
    const res = await axios.get('http://localhost:3000/api/Products/cocktails')
    return{
        props:{
            drinksdata:res.data
        }
